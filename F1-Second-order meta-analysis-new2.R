...
aa <- read.csv("Overall-second order-new.csv")
yielda <- dplyr::filter(aa,response_variable=="yield")
N2Oa <- dplyr::filter(aa,response_variable=="N2O")

yield.positive <- dplyr::filter(yielda,count=="positive")
yield.neutral <- dplyr::filter(yielda,count=="neutral")
yield.negative <- dplyr::filter(yielda,count=="negative")
N2O.positive <- dplyr::filter(N2Oa,count=="positive")
N2O.neutral <- dplyr::filter(N2Oa,count=="neutral")
N2O.negative <- dplyr::filter(N2Oa,count=="negative")

data1 <- read.csv("Fig.1a-new.csv")
data1$variable <- factor(data1$variable,levels = c("N2O","yield"),labels = c("N2O","Yield"))
data1$response <- factor(data1$response,levels = c("positive","neutral","negative"),
                         labels = c("Significant increase","No change","Significant reduction"))

p1a <- ggplot()+
  geom_col(data=data1,aes(x=variable,y=count,fill=response),position="stack",width=0.5)+
  scale_fill_manual(values=c("#2ECC71","#F1C40F","#E74C3C"))+
  theme_cowplot()+
  theme(panel.grid = element_blank())+
  scale_x_discrete(labels=c(expression(paste(N[2],"O",sep = "")),"Yield"))+
  ylab("Number of effect sizes")+
  theme(legend.background = element_blank(),legend.direction = "vertical")+font+
  coord_flip()+
  theme(legend.title = element_blank(),axis.title.y = element_blank(),legend.position = c(0.4,0.25))+
  labs(subtitle = "Systematic review")
p1a

#####Overall effect######
aa$ID <- as.factor(aa$ID)
aa$number <- as.factor(aa$number)
aa$var <- (aa$se2)^2
a_result <- rma.mv(effect_size2,var,mods=~response_variable-1,data=aa,random=~1|number/ID)

data2 <- read.csv("Fig.1b-new.csv")
data2$variable <- factor(data2$variable,levels = c("N2O","yield"),labels = c("N2O","Yield"))

p2a <- ggplot(data2)+ 
  geom_hline(aes(yintercept=0),linetype="dashed",colour="black")+
  geom_point(aes(x=variable, y=mean, col=variable,shape=sig),stat="identity",size=3.6)+ ##画数据点
  geom_errorbar(aes(x=variable,ymin=CI1, ymax=CI2, col=variable), width=0.2, size=0.5)+ ##画95%CI
  #facet_wrap(~variable,nrow = 1,scales = "free_x")+
  scale_shape_manual(values = c(16,21))+
  scale_x_discrete(labels=c(expression(paste(N[2],"O",sep = "")),"Yield"))+
  geom_text(aes(label=n1, x=variable, y=CI2+0.02,hjust=0), col="black", size=4.2)+  ##添加数据量
  geom_text(aes(label=n2, x=variable, y=CI1-0.02,hjust=1), col="black", size=4.2)+  ##添加数据量
  coord_flip()+
  theme_cowplot()+ theme(panel.background = element_blank())+ ##格式
  scale_colour_manual(values=c("#48C9B0","#F95738")) +
  ...
  theme(axis.title.y = element_blank())+font+
  labs(subtitle = "Second-order meta-analysis")
p2a
...
climate_result$n <- c("(6)","(23)","(23)","(19)","(4)","(7)","(4)","(2)")
...
N2O_texture<- rma.mv(effect_size2,var,data=N2O2,mods=~texture-1,random=~1|ID)
...
texture_result$variable <- factor(texture_result$variable,
                                  levels = c("yield","N2O"),labels = c("Yield","N2O"))
texture_result$name <- factor(texture_result$name,
                              levels = c("Sandy","Loam","Clay loam","Clay"))
texture_result$n <- c("(10)","(6)","(13)","(13)","(1)","(4)","(4)","(4)")
texture_result$sig <- c("b","b","b","a","b","a","b","b")
texture_data$variable <- factor(texture_data$variable,
                                levels = c("yield","N2O"),labels = c("Yield","N2O"))
...
application_result$n <- c("(12)","(5)","(7)","(5)","(3)","(3)")
...
CC.type_result$n <- c("(49)","(21)","(55)","(7)","(3)","(8)")
...
N2O6 <- filter(N2O,N.rate!="")

yield_N.rate <- rma.mv(effect_size2,var,data=yield6,mods=~N.rate-1,random=~1|number/ID)
N2O_N.rate<- rma.mv(effect_size2,var,data=N2O6,mods=~N.rate-1,random=~1|ID)
...
N.rate_N <- orchaRd::mod_results(N2O_N.rate,mod="N.rate",group="number")
N.rate_N_1 <- data.frame(N.rate_N$mod_table)%>%mutate(variable="N2O")
N.rate_N_2 <- data.frame(N.rate_N$data)%>%mutate(precision=1/sqrt(vi),variable="N2O")

N.rate_result <- rbind.data.frame(N.rate_y_1,N.rate_N_1)
N.rate_data <- rbind.data.frame(N.rate_y_2,N.rate_N_2)
N.rate_result$variable <- factor(N.rate_result$variable,
                                 levels = c("yield","N2O"),labels = c("Yield","N2O"))
N.rate_result$name <- factor(N.rate_result$name,
                             levels = c("Low","Recommended","High"))
N.rate_result$n <- c("(13)","(13)","(9)","(1)","(1)","(1)")
N.rate_result$sig <- c("b","a","b","b","b","b")
N.rate_data$variable <- factor(N.rate_data$variable,
                               levels = c("yield","N2O"),labels = c("Yield","N2O"))
...
ggdraw()+ 
  draw_plot(p1a, x=0, y=0.5, width = 1/3, height = 0.5)+
  draw_plot(p2a, x=0, y=0, width = 1/3, height = 0.5)+
  draw_plot(p8, x=1/3-0.005, y=0, width = 2/3, height = 1)+
  draw_plot_label(label = c("a","b","c","d","e","f","g","h"), size = 15,
                  x=c(0.01,0.01,1/3-0.01,1/3-0.01,1/3-0.01,2/3-0.07,2/3-0.07,2/3-0.07),
                  y=c(1,0.5,1,2/3,1/3,1,2/3,1/3))##11.6*8.6
